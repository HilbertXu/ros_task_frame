...
float TargetShift::normalizeAngle(float angle) {
  float res = angle;
  if (angle > PI) {
    res -= 2.0 * PI;
  } else if (angle < -PI) {
...
float TargetShift::quatToAngle(tf::Quaternion quat) {
...
tf::StampedTransform TargetShift::getOdomTransform() {
  tf::StampedTransform transform;
  try
  {
    pListener.waitForTransform(odomFrame_, baseFrame_, ros::Time(), ros::Duration(1.0));
...
void TargetShift::moveBaseFocus(float goalRadius, float startAngle) {
  float goal_;
  ...
  float lastAngle_ = startAngle;
  float turnAngle_ = 0;

  if (abs(turnAngle_ + angularTolerance_) < abs(goal_)) {
