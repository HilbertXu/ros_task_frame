...
#include <robot_vision_msgs/ObjectPosition.h>
#include <robot_control_msgs/Mission.h>
#include <robot_control_msgs/Result.h>
...
private:
    ...
    void controlCallback(const robot_control_msgs::Mission msg)
    {
        if (msg.action == "grasp")
        {
            // 从control节点处获得带抓取物体的名称
            if (msg.target == "object")
            {
                object_name = msg.attribute;
            }
        }
    }
    ...
    void get_object_position()
    {
        ...
        else
        {
            ...
            robot_vision_msgs::ObjectPosition pos;
...
public:
    int run (int argc, char** argv)
    {
        ...
        obj_pub = nh.advertise<robot_vision_msgs::ObjectPosition>(pub_object_pos_topic_name, 1);
